...
**上游仓库**：`https://github.com/pbeenigg/LittleCrawler.git`

## 前置检查

- 当前仓库根目录为 LittleCrawler 项目（或用户明确指定了项目路径）。
...
- 执行 `git remote -v`，若尚未存在指向 `github.com/pbeenigg/LittleCrawler` 的 remote：
  - 询问用户：「是否将上游添加为 remote（例如命名为 `upstream`）？」
  - 若用户同意，执行：`git remote add upstream https://github.com/pbeenigg/LittleCrawler.git`
...
- 执行 `git remote add upstream https://github.com/pbeenigg/LittleCrawler.git`（若尚未添加）。
